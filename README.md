# ربات مانیتور دیوار + سکه

مانیتور آگهی‌های دیوار و قیمت سکه (امامی، نیم، گرمی از TGJU) با اطلاع از طریق تلگرام.
...
1. یک Environment به نام `env-find-my-car` بسازید (Settings → Environments) و این Secrets را داخل آن بگذارید:
...
Workflow با `environment: env-find-my-car` به همین Secrets وصل است؛ اگر Environment اسم دیگری دارد، همان را در `.github/workflows/monitor.yml` عوض کنید.

2. Actions را در ریپو فعال کنید؛ workflow هر حدود ۱۰ دقیقه یک بار (و با `workflow_dispatch` دستی) یک cycle با `RUN_ONCE=1` اجرا می‌کند و در صورت تغییر، `data/posts.json` و `data/coins.json` را commit می‌کند تا ران بعدی cold-start نشود.
...
در هر cycle، همزمان با اسکرپ دیوار، صفحهٔ [tgju.org/coin](https://www.tgju.org/coin) هم خوانده می‌شود و قیمت نقدی این‌ها استخراج می‌شود:

- سکه امامی (`sekee`)
- نیم سکه (`nim`)
- سکه گرمی (`gerami`)

وضعیت در `data/coins.json` ذخیره می‌شود. در اولین اجرا یک اسنپ‌شات تلگرامی ارسال می‌شود؛ بعد از آن فقط در صورت تغییر قیمت نوتیف می‌آید.

## نکات

- در اجرای اول (وقتی `data/posts.json` خالی است) فقط وضعیت فعلی آگهی‌ها ذخیره می‌شود و نوتیف سیل آگهی‌های موجود ارسال نمی‌شود؛ از دور بعد آگهی‌های جدید/تغییر قیمت اطلاع داده می‌شوند.
- فاصلهٔ پیشنهادی بین بررسی‌ها حداقل ۳–۵ دقیقه است تا rate limit دیوار کمتر پیش بیاید.
- استفاده شخصی و مودبانه؛ رعایت شرایط استفادهٔ دیوار و TGJU بر عهدهٔ شماست.
